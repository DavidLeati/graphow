...
2. **Orientar-se na tarefa.** `ler_vista` na Task assumida e, se faltar detalhe de vizinho, `expandir_no` no nó específico. Leia primeiro `Aprendizados Aplicaveis`, quando a seção vier: é o que outras sessões e outros projetos já aprenderam sobre isto, com a origem de cada afirmação. As linhas que casam com o texto da tarefa vêm inteiras, com `como aplicar` e origem; as demais trazem só a afirmação, e `expandir_no` no id traz o resto. `Decisoes Que Governam Esta Tarefa` traz as decisões ligadas por `orienta` à tarefa ou a quem a contém; `Perto Desta Tarefa, Sem Governa-la` é o que a sessão registrou para outras tarefas, e é contexto, não instrução.
