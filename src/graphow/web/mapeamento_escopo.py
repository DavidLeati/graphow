...
# profundidade fixa quebra em silêncio se ela crescer.
...
class MapeadorEscopo:
    ...
    def mapear_projetos(self, view: GrafoView) -> Mapping[str, str]:
        ...
        return self._propagar_filiacao(view, TipoNo.PROJETO)

    def mapear_setores(self, view: GrafoView) -> Mapping[str, str]:
        """Propaga a filiação a setor pela subárvore de cada Setor, até o ponto fixo.

        É a mesma dobra do projeto, começando um nível abaixo: o Setor alcança as
        Sessões que contém e, por elas, o trabalho que produziram. O Projeto fica
        de fora porque a propagação só desce.
        """
        return self._propagar_filiacao(view, TipoNo.SETOR)

    def _propagar_filiacao(self, view: GrafoView, tipo_raiz: TipoNo) -> dict[str, str]:
        """Parte de cada contêiner do tipo dado e desce pelas arestas de contenção."""
        mapa: dict[str, str] = {no.id: no.id for no in view.listar_nos_por_tipo(tipo_raiz)}
