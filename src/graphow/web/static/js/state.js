...
const RECORTE_VAZIO = { colapsar: "", escopo: "", raio: 1, vista: "" };
...
export class GraphowState {
  constructor() {
    ...
    this.hideStructuralEdges = lerPreferencia("ocultar_arestas_sessao", true);
    this.mostrarMinimapa = lerPreferencia("minimapa", true);
    // O recorte do canvas e resolvido no servidor: aqui fica so o que pedir.
    this.recorte = { ...RECORTE_VAZIO, ...lerPreferencia("recorte", {}) };
    this.recorteAplicado = null;
    this.listeners = new Set();
    this.layoutPersistTimer = null;
  }

  definirRecorte(mudanca) {
    this.recorte = { ...this.recorte, ...mudanca };
    gravarPreferencia("recorte", this.recorte);
  }

  definirEscopo(escopo) {
    this.escopo = escopo || null;
  }
  ...
  parametrosDeEscopo() {
    ...
    if (escopo.tipo !== "Sessao") return "";
    return `&sessao=${id}${escopo.projetoId ? `&projeto=${encodeURIComponent(escopo.projetoId)}` : ""}`;
  }

  alternarMinimapa() {
    this.mostrarMinimapa = !this.mostrarMinimapa;
    gravarPreferencia("minimapa", this.mostrarMinimapa);
  }

  // A query so carrega o que foi de fato pedido: um parametro vazio faria o
  // servidor decidir por omissao, e o padrao dele e devolver o grafo inteiro.
  parametrosDeRecorte() {
    const r = this.recorte;
    const partes = [];
    if (r.colapsar) partes.push(`colapsar=${encodeURIComponent(r.colapsar)}`);
    if (r.escopo) partes.push(`escopo=${encodeURIComponent(r.escopo)}&raio=${r.raio}`);
    if (r.vista) partes.push(`vista=${encodeURIComponent(r.vista)}`);
    return partes.length ? `&${partes.join("&")}` : "";
  }

  toggleStructuralEdges() {
    this.hideStructuralEdges = !this.hideStructuralEdges;
    gravarPreferencia("ocultar_arestas_sessao", this.hideStructuralEdges);
  ...
  setCanvasData(data) {
    ...
    this.recorteAplicado = data.recorte || null;
