/**
 * Ícones da moldura, no traço do Lucide (licença ISC).
 *
 * Os ícones vivem aqui como SVG em linha, e não como fonte ou arquivo externo: a
 * interface roda local e offline, e um ícone que depende de rede vira um
 * quadrado vazio justamente quando a pessoa está sem ela.
 */

const TRACOS = {
  search: '<circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/>',
  command: '<path d="M15 6v12a3 3 0 1 0 3-3H6a3 3 0 1 0 3 3V6a3 3 0 1 0-3 3h12a3 3 0 1 0-3-3"/>',
  grafo: '<circle cx="6" cy="6" r="2.5"/><circle cx="18" cy="8" r="2.5"/><circle cx="9" cy="18.5" r="2.5"/><path d="M8.5 6.4 15.5 7.6"/><path d="M6.6 8.4 8.4 16"/><path d="M16.6 10.1 10.8 16.6"/>',
  ...
  ordenar: '<path d="m3 8 4-4 4 4"/><path d="M7 4v16"/><path d="M11 12h4"/><path d="M11 16h7"/><path d="M11 20h10"/>',
  ...
  "square-check": '<path d="M21 10.5V19a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h12.5"/><path d="m9 11 3 3L22 4"/>',
  ...
  "circle-half": '<circle cx="12" cy="12" r="10"/><path d="M12 2a10 10 0 0 1 0 20z" fill="currentColor"/>',
...
/**
 * Devolve o SVG do ícone pedido. Um nome desconhecido vira um círculo em vez de
 * nada: um botão sem ícone some da vista, um círculo pelo menos denuncia o erro.
 */
export function icone(nome, { tamanho = 16, classe = "" } = {}) {
  const traco = TRACOS[nome] || TRACOS.circle;
  return `<svg class="svg-icone ${classe}" width="${tamanho}" height="${tamanho}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${traco}</svg>`;
}

/** Preenche todo `[data-icone]` do trecho com o SVG correspondente. */
export function hidratarIcones(raiz = document) {
  raiz.querySelectorAll("[data-icone]").forEach((alvo) => {
    const tamanho = Number(alvo.dataset.iconeTamanho || 16);
    alvo.insertAdjacentHTML("afterbegin", icone(alvo.dataset.icone, { tamanho }));
    alvo.removeAttribute("data-icone");
  });
}
