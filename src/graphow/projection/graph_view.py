...
from graphow.projection.caminho_critico import CalculadoraDeCaminhoCritico, CaminhoCritico
from graphow.projection.ranking_busca import CriterioBusca, ResultadoDaBusca, ranquear
from graphow.projection.rollup import IndiceDeRollup, ResumoDeSubarvore
from graphow.projection.working_set import RAIO_PADRAO, CalculadoraDeEscopoAtivo, EscopoAtivo


class GrafoView:
    """Consultas somente-leitura sobre o estado projetado do grafo em memória."""

    def __init__(self, estado: GrafoEstado, indice: IndiceDeRollup | None = None) -> None:
    ...
    def indice_de_rollup(self) -> IndiceDeRollup:
        ...
        A projeção sincronizada já o entrega calculado por commit. O cálculo
        preguiçoso existe para quem monta a view direto sobre um estado, como
        os testes e as ferramentas de linha de comando.
        """
        if self._indice is None:
            self._indice = IndiceDeRollup.calcular(self._estado)
        return self._indice

    def obter_resumo(self, id_no: str) -> ResumoDeSubarvore | None:
        """Resumo agregado da subárvore do nó, ou None se ele nada contiver."""
        return self.indice_de_rollup.obter(id_no)

    def calcular_escopo_ativo(self, raio: int = RAIO_PADRAO) -> EscopoAtivo:
        """Recorte do grafo em torno do trabalho que ainda não terminou."""
        return CalculadoraDeEscopoAtivo(self._estado).calcular(raio)

    def calcular_caminho_critico(self) -> CaminhoCritico:
        """Subgrafo de dependências, com os gargalos ordenados por alcance."""
        return CalculadoraDeCaminhoCritico(self._estado).calcular()
    ...
    def obter_filhos_por_contencao(self, id_no: str) -> list[NoGrafo]:
        """Filhos diretos do nó pelas arestas de contenção da ontologia."""
        ids = [
            aresta.destino_id
            for aresta in self._estado.arestas.values()
            if aresta.origem_id == id_no and aresta.tipo in ARESTAS_DE_CONTENCAO
        ]
        unicos = sorted(dict.fromkeys(ids))
        return [self._estado.nos[id_filho] for id_filho in unicos if id_filho in self._estado.nos]
    ...
    def buscar_ranqueado(
        self,
        criterio: CriterioBusca,
        escopo: EscopoAtivo | None = None,
    ) -> ResultadoDaBusca:
        """Busca ordenada por relevância e cortada no limite pedido.

        `buscar_nos` continua devolvendo tudo, sem ordem definida: ela é o
        primitivo, e há chamadores que dependem do conjunto inteiro. O escopo,
        quando informado, restringe os candidatos antes do ranking — filtrar
        depois deixaria a página com menos itens do que o limite pedido.
        """
        candidatos = self._filtrar_por_tipos(criterio.tipos)
        if escopo is not None and not escopo.esta_vazio:
            candidatos = tuple(no for no in candidatos if escopo.contem(no.id))
        return ranquear(candidatos, criterio)

    def _filtrar_por_tipos(self, tipos: Sequence[TipoNo]) -> tuple[NoGrafo, ...]:
        """Nós dos tipos informados, ou todos quando nenhum tipo foi pedido."""
        if not tipos:
            return tuple(self._estado.nos.values())
        aceitos = frozenset(tipos)
        return tuple(no for no in self._estado.nos.values() if no.tipo in aceitos)
