...
| :--- | :--- | :--- |
| `CAMADAS_POR_NIVEL` | `Mapping[str, tuple[TipoNo, ...]]` | `{'projeto': (TipoNo.PROJETO,), 'setor': (TipoNo.PROJETO, TipoNo.SETOR),…` |
| `NIVEIS_ACEITOS` | `tuple[str, ...]` | `tuple(CAMADAS_POR_NIVEL)` |

### `OpcoesDeRecorteVisual`

*DTO imutável* — Os três recortes que a tela pode pedir, combináveis entre si.

**Campos:** `colapsar_em: str`, `escopo_ativo: bool`, `raio: int`, `caminho_critico: bool`

- `nivel_de_colapso() -> str` `[property]` — Nível saneado, ou vazio quando o pedido não nomeia um nível conhecido.
- `pede_algum_recorte() -> bool` `[property]` — Indica se há qualquer filtro a aplicar sobre o grafo completo.

### `RecortadorVisual`

*serviço* — Aplica os recortes pedidos e devolve os identificadores que sobrevivem.

- `selecionar(opcoes: OpcoesDeRecorteVisual) -> SelecaoVisual` — Interseca os recortes pedidos sobre o conjunto de nós do grafo.

### `SelecaoVisual`

*DTO imutável* — Quais nós a tela deve receber, e a explicação do que ficou de fora.

**Campos:** `ids: frozenset[str]`, `total_no_grafo: int`, `filtros_aplicados: tuple[str, ...]`, `nos_orfaos: tuple[str, ...]`, `detalhes: Mapping[str, Any]`

- `total_oculto() -> int` `[property]` — Quantos nós do grafo não vão para a tela neste recorte.
- `em_dicionario() -> dict[str, Any]` — Bloco `recorte` da resposta, para a interface explicar o que escondeu.
...
**Campos:** `termo: str`, `tipos: tuple[str, ...]`, `limite: int`, `ramo_id: str`
...
Mapeamento de cada nó do grafo à Sessão, ao Setor e ao Projeto que o contêm.
...
- `mapear_setores(view: GrafoView) -> Mapping[str, str]` — Propaga a filiação a setor pela subárvore de cada Setor, até o ponto fixo.
...
*serviço* — Busca textual ranqueada sobre o grafo inteiro do ramo.

- `buscar(req: RequisicaoBusca) -> dict[str, Any]` — Ranqueia, corta no limite e diz quantos existem no total.
...
- `obter_canvas(ramo_id: str, sessao_id: str | None, projeto_id: str | None) -> DadosCanvasVisual` — Gera a projeção visual do grafo, já recortada pelo que a tela pediu.
- `criar_no(req: RequisicaoNovoNo) -> RespostaReciboWeb` — Processa a criação de um novo nó com vinculação opcional à Sessão ou ao contêiner pai.
