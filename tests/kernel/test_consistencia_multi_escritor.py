...
Reproduzem, como propriedades verificáveis, dois bugs de consistência entre
escritores: um cache que não reconsulta o log, e um commit que não era
transacional.
"""
...
def test_segundo_escritor_enxerga_bloqueio_criado_pelo_primeiro(tmp_path: Path) -> None:
    """Dois kernels sobre o mesmo banco nao podem divergir."""
...
def test_lote_rejeitado_nao_persiste_evento_algum(tmp_path: Path) -> None:
    """Um lote que colide na sequencia nao pode deixar metade dos eventos gravada."""
    caminho = tmp_path / "graphow.db"
